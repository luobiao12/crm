...
class ShowList(object):
    def __init__(self, config, data_list, request):
        self.config = config  #ModelStark中的self
    ...
    def get_body(self):
        # 构建表单数据
        new_data_list = []  #
        # [['<input type="checkbox">', 1, '小说', "<a href='/stark/app01/book/1/change/'>编辑</a>", "<a href='/stark/app01/book/1/delete/'>删除</a>"],
        # ['<input type="checkbox">', 2, '英文', "<a href='/stark/app01/book/2/change/'>编辑</a>", "<a href='/stark/app01/book/2/delete/'>删除</a>"]]
...
class ModelStark(object):
    ...
    def get_modelform_class(self):
        ####获取ModelForm类，如果自己定制用自己的，没有定制的话，就用这个空的类
    ...
    def get_urls_2(self):
        temp = []
        model_name = self.model._meta.model_name  # 取出表名
        app_label = self.model._meta.app_label  # 取出app名
...
class StarkSite(object):
    ...
    def get_urls(self):  # 一级路由分发
        temp = []
        for model, stark_class_obj in self._registry.items():
            model_name = model._meta.model_name  # 取出表名
            app_label = model._meta.app_label  # 取出app名
            temp.append(url(r"^{}/{}/".format(app_label, model_name), stark_class_obj.urls_2))  # 分发增删改查二级路由分开
        return temp
